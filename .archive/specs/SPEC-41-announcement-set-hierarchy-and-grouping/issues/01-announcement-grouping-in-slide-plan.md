...
- [x] Runtime Contract (`src/lib/artifacts/runtime-contract.ts` and `src/lib/artifacts/preview-model.ts`):
...
- [x] TypeScript Slide Planner (`src/lib/slide-plan.ts`):
...
- [x] Go Slide Planner & Snapshot (`internal/plan/plan.go`, `internal/plan/snapshot.go`, `internal/plan/types.go`):
