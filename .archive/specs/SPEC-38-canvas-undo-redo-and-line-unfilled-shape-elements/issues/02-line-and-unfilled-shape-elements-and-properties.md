...
- [x] Extend data models and types:
...
- [x] Extend server and client validation:
...
- [x] Fabric.js rendering and shared serialization in `src/lib/registry/canvas-utils.ts`:
...
- [x] Toolbar creation tools in `ArtifactEditor.tsx`:
...
- [x] Right-click context menu and duplicate parity:
...
- [x] Element properties inspector (Toolbar Row 2):
...
- [x] Add unit tests in `tests/smoke-spec-38.test.mjs` verifying TypeScript and Go schema acceptance/rejection (testing valid values, `h: 0` for lines, and rejecting invalid negative stroke widths), and verifying selection hit-testing for thin lines and transparent center of outline shapes.
