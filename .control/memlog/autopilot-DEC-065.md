...
- Stopped at: Mandate DEC-065 completed — SPEC-70 and SPEC-71 delivered through G5 Release
- Blocked: —
- Parked: —
- Next: Maintainer review and merge of PR #108
...
| I-3 (SPEC-71-01) | parser.go & parser-rules.ts | Support rawText fallback for dynamic song set regex extraction when line-by-line produces no positive match, enabling section-scoped multiline dotall patterns without collisions, verified by Terra peer review and cross-layer tests | failing to extract identically labeled songs in multi-section worship bulletins | medium | internal/parse/parser.go, src/lib/parser-rules.ts, src/components/admin/FormLayoutAdminPanel.tsx, internal/parse/dynamic_extraction_test.go, internal/httpapi/services_field_values_test.go, tests/dynamic-field-extraction.test.mjs, .scratch/SPEC-71-section-scoped-multiline-song-set-regex-extraction/issues/01-support-rawtext-fallback-for-song-set-regex-extraction.md, .control/registry/specs.yaml |

## Smoke Test Results

- FR-32 (Dynamic Song Set Suggestions & Regex Extraction): PASS — multi-section and multiline dotall patterns extract songs without collisions and with zero false overflow banners.
- FR-36 (Parser Configuration & Profile Retirement): PASS — legacy parser profiles completely retired from UI and backend, intake decoupled to static liturgy with dynamic predefined fields and song set regexes.
