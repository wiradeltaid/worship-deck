...
# DEC-046 — Daily Autopilot mandate for continuous engineering routine and delivery (SPEC-45)

## Decision

> The owner grants an autonomous daily engineering execution mandate under DEC-046:
...
> 5) Worktree isolation & Integration: coordinator alone isolates worktrees, logs all decisions in `.control/memlog/autopilot-DEC-046.md`, stages, commits, and merges per `wdi-autopilot`;
> carrying implementation through G5 Release in the run branch `autopilot/DEC-046`.

## Why

To allow unattended continuous execution of engineering tickets and backlog releases (SPEC-45 announcement slot single-row layout and upload persistence hydration) with high fidelity and strict verification without blocking on manual prompts.
