...
# DEC-044 — Daily Autopilot mandate for continuous engineering routine and delivery (SPEC-43)

## Decision

> The owner grants an autonomous daily engineering execution mandate under DEC-044:
...
> 5) Worktree isolation & Integration: coordinator alone isolates worktrees, logs all decisions in `.control/memlog/autopilot-DEC-044.md`, stages, commits, and merges per `wdi-autopilot`;
> carrying implementation through G5 Release in the run branch `autopilot/DEC-044`.

## Why

To allow unattended continuous execution of engineering tickets and backlog releases (SPEC-43 presenter remote scripture announcement ux) with high fidelity and strict verification without blocking on manual prompts.
