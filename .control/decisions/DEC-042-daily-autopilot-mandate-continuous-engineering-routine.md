...
# DEC-042 — Daily Autopilot mandate for continuous engineering routine and delivery (SPEC-39)

## Decision

> The owner grants an autonomous daily engineering execution mandate under DEC-042:
...
> 5) Worktree isolation & Integration: coordinator alone isolates worktrees, logs all decisions in `.control/memlog/autopilot-DEC-042.md`, stages, commits, and merges per `wdi-autopilot`;
> carrying implementation through G5 Release in the run branch `autopilot/DEC-042`.

## Why

To allow unattended continuous execution of engineering tickets, backlog items, or bug fixes (starting with SPEC-39 media gallery, presenter announcement section looping, and dynamic announcement placeholders) with high fidelity and strict verification without blocking on manual prompts.
