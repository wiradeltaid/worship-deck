...
- [x] Read `src/components/ImageUploadField.tsx` and the ImageCropDialog component first.
- [x] In `src/components/ImageUploadField.tsx`:
...
- [x] Author `tests/service-image-crop-integration.test.mjs`:
...
- [x] Wire `node --import ./tests/register-ts-resolve.mjs --test tests/service-image-crop-integration.test.mjs` into `package.json` test script additively preserving `--test-concurrency=1`.
