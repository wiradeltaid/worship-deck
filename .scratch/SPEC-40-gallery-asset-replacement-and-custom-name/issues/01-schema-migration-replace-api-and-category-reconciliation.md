...
- [ ] Database Schema & Migration (`internal/db/schema.sql` and `internal/db/migrate.go`):
...
- [ ] Backend Domain Category & Name Parity (`internal/httpapi/background_library.go`):
...
- [ ] Atomic In-Place Asset Replacement Endpoint (`internal/httpapi/background_library.go` and `internal/httpapi/uploads.go`):
