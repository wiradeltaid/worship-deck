...
- [x] Reconcile and extend dynamic placeholders in `src/lib/registry/placeholder-catalog.ts` and Go `internal/plan/plan.go`:
...
- [x] Add `afternoon_program` to weekly worship service record:
...
- [x] Weekly data hydration in `src/lib/slide-plan.ts` and `internal/plan/hydrate.go`:
...
- [x] PPTX Export Resolution in `src/lib/pptx-draw.ts`:
