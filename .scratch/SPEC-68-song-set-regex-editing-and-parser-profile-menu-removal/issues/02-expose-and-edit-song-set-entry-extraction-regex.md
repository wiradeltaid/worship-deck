...
- [x] Read `internal/httpapi/song_set_entries.go`, `src/components/admin/SongSetEntriesPanel.tsx`, and `src/components/admin/FormLayoutAdminPanel.tsx` in full first.
- [x] In `internal/httpapi/song_set_entries.go`:
...
- [x] In `src/components/admin/SongSetEntriesPanel.tsx`:
...
- [x] End-to-End Dynamic Parsing & Suggestions:
...
- [x] Add regression tests in `tests/dynamic-field-extraction.test.mjs` or a dedicated test suite verifying:
