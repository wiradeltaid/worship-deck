...
1. Disable Webhook in Code (Owner Decision Q1):
   - In `internal/httpapi/webhook.go`: ensure `POST /api/webhook` remains a registered route (maintaining routing topology and session-free gate allowance), but unconditionally returns an HTTP error (e.g. 503 Service Unavailable or 403 Forbidden with message "Webhook intake is disabled in this release") immediately before reading the request body.
...
3. Testing:
   - In Go test `internal/httpapi/webhook_test.go`: assert that sending a valid webhook payload with `X-Webhook-Secret` returns an error, creates no service records, and does not read from the request body (proven using a specialized `io.Reader` that records reads).
   - In `tests/webhook-auth.test.mjs`: update client test assertions to verify that setup does not write `WEBHOOK_SECRET` and requests are cleanly refused without unhandled server panics.
...
- [ ] In `internal/httpapi/webhook.go`: return immediate error before reading request body.
...
- [ ] Add Go test in `internal/httpapi/webhook_test.go` confirming body is unread and request rejected. Verify red first, then green.
