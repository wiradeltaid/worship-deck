...
test('T-27-02: Line spacing normalization reflects tight (<1.0) and wide (>1.0) line-heights accurately', async () => {
  ...
  assert.ok(slide1Xml, 'ppt/slides/slide1.xml must exist');
  // Tight line-heights (< 1.0) use calibrated proportional pitch (lineHeight * 0.85):
  // Math.round((0.8 * 0.85) * 100000) = 68000, matching CSS line-height without
  // upward compression or excessive gap in PowerPoint.
  assert.ok(
    slide1Xml.includes('<a:spcPct val="68000"/>'),
    'slide XML must contain <a:spcPct val="68000"/> for 0.8 line-height'
  );
});
