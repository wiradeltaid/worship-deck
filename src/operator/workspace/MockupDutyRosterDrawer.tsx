...
export default function MockupDutyRosterDrawer({
  ...
  const [newPersonRole, setNewPersonRole] = useState('Volunteer');

  const handleSelect = (person: RosterPerson) => {
    onSelectSpeaker(person.name);
    toast.success(`Selected volunteer: ${person.name}`);
    onOpenChange(false);
  };
  ...
  const handleAddNewPerson = (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      toast.error('Volunteer name cannot be empty');
      return;
    }

    const newPerson: RosterPerson = {
      id: `r-${Date.now()}`,
      name: `${trimmed} (Synthetic)`,
      role: newPersonRole.trim() || 'Volunteer',
      assigned: false,
    };

    setRoster((prev) => [...prev, newPerson]);
    setNewPersonName('');
    toast.success(`New volunteer added to roster: ${newPerson.name}`);
  };
        ...
        <div className="space-y-4 overflow-y-auto py-1">
          <div className="p-3 bg-muted/40 rounded-xl border border-border/70 space-y-1">
            <p className="text-xs font-semibold text-foreground flex items-center gap-1.5">
              ...
              <span>Registered Volunteers (Synthetic Demo Data)</span>
            </p>
            <p className="text-[11px] text-muted-foreground">
              Selecting a volunteer directly binds their name to the sermon speaker field and token{' '}
          ...
          >
            <Label className="text-xs font-semibold flex items-center gap-1.5">
              ...
              <span>Add New Volunteer to Roster</span>
            </Label>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="sm:col-span-2">
                <Input
                  placeholder="Volunteer name (e.g. Timothy Silas)"
                  value={newPersonName}
