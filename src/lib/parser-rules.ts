...
export function cleanLineWithProfile(line: string, profile?: ParserProfileRules | null): string {
...
export function parseScriptureValueWithProfile(
  ...
  const refPattern =
    profile?.scripture_ref_pattern ?? '^(?<book_chapter>.+?\\s+\\d+:[\\d,\\-–]+)\\s*$';
      ...
      for (const hp of hymnPatterns) {
        ...
        if (m) {
          ...
          if (numStr) {
            ...
            let found: { title: string; lyrics: string; incomplete?: boolean };
            if (hymnLookup) {
              found = hymnLookup(number, bookCode);
            } else {
              found = { title: `${bookCode} ${number}`, lyrics: '', incomplete: false };
            }
