...
| 13 | spa/WorkspaceMockupPage | `/new` | — | hub | — |
...
- **Row 13 (`spa/WorkspaceMockupPage`) has no UC served, and this is deliberate, not a gap.** Confirmed the same day: zero `fetch()` calls anywhere in `src/operator/workspace/` — it is a client-side UI prototype over synthetic in-memory data, not a build wired to any real promise. An earlier version of this row named UC-5, inherited loosely from `ServiceRunSheet`'s own UC list; that mapping is corrected here rather than left standing, since it would have claimed this prototype delivers a real Hub promise it does not.
